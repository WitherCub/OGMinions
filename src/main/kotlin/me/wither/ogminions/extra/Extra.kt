package me.wither.ogminions.extra
...
class NBT {
    companion object {
        fun addType(item: ItemStack, type: String): ItemStack {
            return NBTEditor.set(item, type, "OGMinionsType")
        }

        fun addItem(item: ItemStack, type: ItemType): ItemStack {
            return NBTEditor.set(item, type.toString(), "OGMinionsItem")
        }
        ...
        fun hasMetadata(item: ItemStack?): Boolean {
            return try {
                NBTEditor.contains(item, "OGMinionsItem")
            } catch (e: NullPointerException) {
                false
            }
        }
        ...
        fun getType(item: ItemStack): String {
            return NBTEditor.getString(item, "OGMinionsType")
        }

        fun getItem(item: ItemStack): ItemType? {
            return try {
                ItemType.valueOf(NBTEditor.getString(item, "OGMinionsItem"))
