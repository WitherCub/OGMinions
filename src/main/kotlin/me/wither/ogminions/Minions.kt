...
import me.wither.ogminions.tasks.StartAnimation
...
open class MinionEntity (
    ...
    fun performAnimation() {
        ...
        if(!animationStatus) {
            ...
            animationTaskID = Bukkit.getScheduler().scheduleSyncRepeatingTask(OGMinionsPlugin.get(), {
                performAnimation()
    ...
    fun startDeathClock() {
        if(hunger <= 0) {
            if(healthDepletionTask == -1) {
                ...
                healthDepletionTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(OGMinionsPlugin.get(), {
                    if(health <= 0) {
                        despawn()
